    ...
    >
      <Drawer.Portal>
        ...
        >
          ...
          >
            <Drawer.Handle className="mx-auto mb-3 !bg-white/30" />
            <div className="pr-10">
              <Drawer.Title className="text-2xl font-bold text-white tracking-tight">
                {format(event.start, 'HH:mm')} – {format(event.end, 'HH:mm')}
              </Drawer.Title>
              <Drawer.Description className="text-sm font-medium text-white/80 mt-0.5">
                {event.title}
              </Drawer.Description>
            </div>
            <Drawer.Close asChild>
              <button className="absolute top-3 right-4 w-8 h-8 rounded-xl bg-white/80 hover:bg-white shadow-md border border-gray-200 text-gray-700 flex items-center justify-center transition-colors">
                <X className="h-4 w-4" />
              </button>
            </Drawer.Close>
          </div>
